...
import { MoveInfoSchema, RawMoveInfoSchema, RawPokemonInfoSchema } from "../../schemas";
import { getMove } from "./moves";
import { getPokemon } from "./pokemon";

const getSafeName = (moveName: string) => moveName.replace(" ", "-").toLowerCase();

export const getMoveByName = async (moveName: string): Promise<MoveInfo | null> => {
	try {
		const res = await fetch(`https://pokeapi.co/api/v2/move/${getSafeName(moveName)}`);
...
export const getMoveByNameWithCache = async (moveName: string): Promise<MoveInfo | null> => {
...
export const getMovesByName = async (movesArray: string[]): Promise<MoveInfo[]> => {
...
export const getPokemonByName = async (name: string): Promise<PokedexEntry | null> => {
	try {
		const res = await fetch(`https://pokeapi.co/api/v2/pokemon/${getSafeName(name)}`);
...
export const getPokemonByNameWithCache = async (name: string): Promise<PokedexEntry | null> => {
