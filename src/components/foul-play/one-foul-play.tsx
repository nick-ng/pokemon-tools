...
export default function OneFoulPlay({
	...
	const foulPlayIV = Math.min(...Object.values(damageCalcs).map((dc) => dc.highestInput));

	const { highestInput: confusionIV, lowestOutput: confusionDamage } = getLargestMin(
		0,
		31,
		getDamageCalc(40, 1, 1)
	);

	const catchDecrease = {
		max: otherStat(catchBaseAtk || baseAtk, catchLevel, foulPlayIV, 0, 0.9),
		over: otherStat(catchBaseAtk || baseAtk, catchLevel, foulPlayIV + 1, 0, 0.9),
	};
	...
	const catchIncrease = {
		...
		over: otherStat(catchBaseAtk || baseAtk, catchLevel, foulPlayIV + 1, 0, 1.1),
	};
	...
	return (
		<div className="border-default m-1 inline-block p-1 align-top">
			...
			<table className="m-auto">
				...
				<tbody>
					{IV_RANGE.filter((iv) => iv <= Math.max(maxIV, foulPlayIV + EXTRA_IV)).map((iv) => {
						const decreaseCatchStat = otherStat(catchBaseAtk || baseAtk, catchLevel, iv, 0, 0.9);
						const neutralCatchStat = otherStat(catchBaseAtk || baseAtk, catchLevel, iv, 0, 1);
						const increaseCatchStat = otherStat(catchBaseAtk || baseAtk, catchLevel, iv, 0, 1.1);
						return (
							<tr
								className={
									iv === foulPlayIV || iv === confusionIV ? "bg-blue-100 dark:bg-blue-900" : ""
								}
								key={iv}
							>
								...
								<td className="border-default px-2 py-0 text-center">
									{Math.round(
										getDamageCalc(95, 1.5, getTypeEffectiveness("dark", [type1, type2]))(iv) * 10
									) / 10}
								</td>
								<td
