...
const getHighestAcceptableIV = (baseStat: number, level: number, nature: number) => {
...
export default function OneIV({
	...
	const catchDecrease = {
		max: otherStat(catchBaseStat || baseStat, catchLevel, highestAcceptableIV, 0, 0.9),
		over: otherStat(catchBaseStat || baseStat, catchLevel, highestAcceptableIV + 1, 0, 0.9),
	};

	const catchDecreaseWarn = catchDecrease.max === catchDecrease.over ? catchDecrease.over : -1;

	const catchNeutral = {
		max: otherStat(catchBaseStat || baseStat, catchLevel, highestAcceptableIV, 0, 1),
		over: otherStat(catchBaseStat || baseStat, catchLevel, highestAcceptableIV + 1, 0, 1),
	};

	const catchNeutralWarn = catchNeutral.max === catchNeutral.over ? catchNeutral.over : -1;

	const catchIncrease = {
		max: otherStat(catchBaseStat || baseStat, catchLevel, highestAcceptableIV, 0, 1.1),
		over: otherStat(catchBaseStat || baseStat, catchLevel, highestAcceptableIV + 1, 0, 1.1),
	};

	const catchIncreaseWarn = catchIncrease.max === catchIncrease.over ? catchIncrease.over : -1;
	...
	return (
		<div className="border-default m-1 inline-block p-1 align-top">
			...
			<table className="m-auto">
				...
				<tbody>
					{IV_RANGE.filter((iv) => iv <= Math.max(maxIV, highestAcceptableIV + EXTRA_IV)).map(
						(iv) => {
							...
							const neutralCatchStat = otherStat(catchBaseStat || baseStat, catchLevel, iv, 0, 1);
							...
							return (
								<tr className={stat === minStat ? "bg-blue-100 dark:bg-blue-900" : ""} key={iv}>
									...
									<td
										className={`border-default px-2 py-0 text-center ${
											decreaseCatchStat === catchDecreaseWarn ? "bg-red-300 dark:bg-red-700" : ""
										}`}
									>
										{decreaseCatchStat}
									</td>
									<td
										className={`border-default px-2 py-0 text-center ${
											neutralCatchStat === catchNeutralWarn ? "bg-red-300 dark:bg-red-700" : ""
										}`}
									>
										{neutralCatchStat}
									</td>
									<td
										className={`border-default px-2 py-0 text-center ${
											increaseCatchStat === catchIncreaseWarn ? "bg-red-300 dark:bg-red-700" : ""
										}`}
									>
										{increaseCatchStat}
									</td>
