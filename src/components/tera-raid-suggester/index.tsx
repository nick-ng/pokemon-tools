	...
	return (
		<div>
				...
				<div>
					...
					{raidYourPokemon.map((yp, i) => (
						<YourPokemon
							...
							onDelete={() => {
								setOptions({
									raidYourPokemon: raidYourPokemon.filter((a) => a.id !== yp.id),
								});
							}}
						/>
					))}
			...
			<details className="ml-2 inline-block max-w-xl align-top">
				...
				<p className="mt-1">You can use the filter to narrow down what raid Pokemon are shown.</p>
				<p className="mt-1">
					You can customise your Pokemon by expanding the "Your Pokemon" section. Some example
					Pokemon are already there. You should remove the Pokemon you don't have and add Pokemon
					you do have or change the existing Pokemon to better match what you have.
				</p>
				<p className="mt-1">
					Ratings assume neither Pokemon boosts/lowers each others stats. If your Pokemon has a
					boosting move or can lower the raid Pokemon's stats, you can increase power of its main
					move to simulate the difference.
				</p>
				<p className="mt-1">
					All the moves the raid Pokemon and your Pokemon know are compared taking type(s), tera
					type, and stats into account.
				</p>
				<p className="mt-1">
					Defense is the number of attacks your Pokemon can take if the raid Pokemon uses its most
					damaging attack.
				</p>
				<p className="mt-1">
					Attack is higher if your Pokemon deals more damage. You can adjust the bias between attack
					and defense by changing the "Attack Adjustment" value.
				</p>
				<p className="mt-1">The formula I've used is:</p>
				<pre>(attack adjustment * raid Pokemon's HP) / your Pokemon's most damaging move</pre>
				<p className="mt-1">
					Raid Pokemon's moves are from Serebii's{" "}
					<a href="https://serebii.net/scarletviolet/teraraidbattles/4star.shtml" target="_blank">
						4 star
					</a>
					,{" "}
					<a href="https://serebii.net/scarletviolet/teraraidbattles/5star.shtml" target="_blank">
						5 star
					</a>
					, and{" "}
					<a href="https://serebii.net/scarletviolet/teraraidbattles/6star.shtml" target="_blank">
						6 star
					</a>{" "}
			...
			<div>
				{raidPokemon
					.filter((r) => !raidFilter || r.pokemon.toLowerCase().includes(raidFilter.toLowerCase()))
					.map((p) => (
						<MatchupDetails
							key={p.id}
							pokemon={p.pokemon}
