...
import { getMovesByName, getPokemonByNameWithCache } from "../../services/pokedex";
import { capFirst, hpStat, otherStat, damage, getTypeEffectiveness } from "../../utils";
...
export default function MatchupDetails({
	...
	const [pokemonDexEntry, setPokemonDexEntry] = useState<PokedexEntry | null>(null);
	...
	const yourPokemonRatings: {
		...
		const {
			id,
			mainMoves,
			pokemon: { name, finalStats: yFinalStats, types: yTypes, teraType: yTeraType },
		} = yp;
		...
		const yourDamage = Math.max(
			...mainMoves.map(moveDamage(finalStats, [raidTeraType], yFinalStats, yTypes, yTeraType, 100))
		);
		...
		const attackRating = (attackAdjustment || 100) / (finalStats.hp / yourDamage);
			...
			{pokemonDexEntry && (
				<div>
					...
					<span className="capitalize">
						{pokemonDexEntry.abilities.map((a) => a.replace("-", " ")).join(", ")}
					</span>
				</div>
			)}
			{pokemonDexEntry && (
					...
					{yourPokemonRatings
						...
						.map((yp) => (
							<tr key={yp.id}>
								...
								<td className="border-default px-1 text-right">{yp.attackRating.toFixed(2)}</td>
								<td className="border-default px-1 text-right">{yp.defenseRating.toFixed(2)}</td>
