...
export const getTypeEffectiveness = (attackType: PokemonType, defenderTypes: PokemonType[]) => {
...
export const IV_RANGE = Object.freeze(new Array(32).fill(null).map((_, i) => i));
...
export const otherStat = (base: number, level = 50, iv = 31, ev = 0, nature = 1) => {
	const a = (2 * base + (iv + Math.floor(ev / 4))) * level;
...
export const getBaseDamage = (
	...
	return Math.floor(
		OF32(
			Math.floor(OF32(OF32(Math.floor((2 * level) / 5 + 2) * basePower) * attack) / defense) / 50 +
				2
		)
	);
};
